...
class Graph:
    ...
    def getAt(self, fromNode, toNode):
        ...
        if self.lazy:
            ...
            if (fromNode, toNode) not in self.cache or (toNode, fromNode) not in self.cache:
            ...
            if (fromNode, toNode) in self.cache:
...
def fromArr(coords, size):
    theGraph = Graph(range(size), None, None, True, True)
    for i in range(size):
        x, y = coords[i]
